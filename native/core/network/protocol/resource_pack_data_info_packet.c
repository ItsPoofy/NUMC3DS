#include "resource_pack_data_info_packet.h"
    ...
    void *output, mcpe_u32 capacity, mcpe_u32 *length)
{
    ...
    *length = 0;
    ...
    result = mcpe_wire_write_le32(&writer, packet->max_chunk_size);
    if (result) return result;
    result = mcpe_wire_write_le32(&writer, packet->chunk_count);
    if (result) return result;
    result = mcpe_wire_write_le64(&writer, &packet->compressed_pack_size);
    if (result) return result;
    result = mcpe_wire_write_string(&writer, packet->sha256_hash,
                                    packet->sha256_hash_length,
                                    MCPE_MAX_STRING_BYTES);
    if (result) return result;
    ...
    McpeResourcePackDataInfoTarget *packet)
{
    ...
    if (result) return result;
    result = mcpe_wire_read_le32(&reader, &packet->max_chunk_size);
    if (result) return result;
    result = mcpe_wire_read_le32(&reader, &packet->chunk_count);
    if (result) return result;
    result = mcpe_wire_read_le64(&reader, &packet->compressed_pack_size);
    if (result) return result;
    result = mcpe_wire_read_string(&reader, packet->sha256_hash,
                                   packet->sha256_hash_capacity,
                                   MCPE_MAX_STRING_BYTES,
                                   &packet->sha256_hash_length);
    if (result) return result;
